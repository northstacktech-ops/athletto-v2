...
export function useDashboard() {
  ...
  // Painel inicial em UMA chamada (métricas + gráfico + turmas de hoje + alertas).
  async function buscarHome(de?: string, ate?: string, meses = 6) {
  ...
  async function buscarMetricas(de?: string, ate?: string): Promise<{
    data: DashboardMetrics | null
    error: Error | null
  }> {
    // Com intervalo → usa o overload por período; sem intervalo → função do mês.
    const { data, error } = (de || ate)
      ? await supabase.rpc('dashboard_metricas', {
          p_clube_id: getClubId(),
          p_de: de ?? null,
          p_ate: ate ?? null,
        })
      : await supabase.rpc('dashboard_metricas', { p_clube_id: getClubId() })
