import { PLANO_LIMITES, type Clube, type Voucher } from '~/types'
...
export function usePlanoLimites() {
  ...
  function avaliar(uso: number, limite: number | null): {
    ...
    return {
      ...
      mensagem: restante <= 0
        ? `Você atingiu o limite de ${limite} do plano ${clube.value?.plano ?? ''}. Faça upgrade para liberar mais.`
  ...
  function avaliarPlanejamentos(totalPlanejamentosAtivos: number) {
    return avaliar(totalPlanejamentosAtivos, limites.value.planejamentos_ativos)
  }
  function avaliarGestores(totalGestoresAdicionais: number) {
    return avaliar(totalGestoresAdicionais, limites.value.gestores_adicionais)
  }
