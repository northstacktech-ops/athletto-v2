            ...
            <tr v-for="a in atletas" :key="a.id" class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
              <td class="px-4 py-2 sticky left-0 bg-white dark:bg-surface-elevated-dark border-r border-gray-100 dark:border-white/[0.06]">
                <div class="flex items-center gap-2">
                  <div class="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-bold text-white" style="background-color: #3d5afe;">
                    {{ getIniciais(a.nome) }}
                  </div>
...
async function carregar() {
  ...
  loading.value = true
  const { data: as } = await freqComp.atletasDaTurma(turmaId.value)
  atletas.value = (as ?? []).map((x: any) => (x.atletas ? x.atletas : x)) as Atleta[]

  // Buscar registros por atleta da turma no período (mock: filtra direto, real: 1 query por atleta — adequado para volume baixo do MVP)
  const todos: Frequencia[] = []
  for (const a of atletas.value) {
    const { data } = await freqComp.historicoPorAtleta(a.id, de.value, ate.value)
    if (data) todos.push(...data.filter((f) => f.turma_id === turmaId.value))
  }
  registros.value = todos
  loading.value = false
}
