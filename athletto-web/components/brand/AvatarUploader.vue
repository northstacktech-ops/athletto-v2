  ...
  <div class="inline-flex flex-col items-start gap-3">
    <div class="flex items-center gap-4">
      ...
      >
        ...
        <div
          v-else
          class="w-full h-full flex flex-col items-center justify-center gap-1.5 bg-slate-50 dark:bg-white/[0.03] text-slate-400 dark:text-white/40"
        >
          <slot name="placeholder">
            <!-- Ícone + CTA sempre visível (não depende de hover) -->
            <svg :width="size * 0.3" :height="size * 0.3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <rect x="3" y="3" width="18" height="18" rx="2"/>
              <circle cx="8.5" cy="8.5" r="1.5"/>
              <path d="M21 15l-5-5L5 21"/>
            </svg>
            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-white/55">Enviar</span>
          </slot>
        </div>
