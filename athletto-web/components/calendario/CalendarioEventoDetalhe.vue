      ...
      <div class="relative w-full sm:max-w-lg bg-white dark:bg-surface-elevated-dark rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 py-4 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <div v-if="turmasVinculadas.length" class="flex flex-wrap gap-1.5">
              <NuxtLink
                v-for="t in turmasVinculadas"
                :key="t.id"
                :to="`/turmas/${t.id}`"
                class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-slate-100 dark:bg-white/[0.06] text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-white/[0.10]"
              >
                <span>{{ iconeEmoji(t.icone) }}</span>
                {{ t.nome }}
              </NuxtLink>
            </div>
        ...
        <div class="px-6 py-3 border-t border-slate-100 dark:border-white/[0.07] flex justify-between gap-3 bg-slate-50 dark:bg-white/[0.02] shrink-0">
          ...
          <div class="flex gap-2">
            ...
            <NuxtLink
              v-if="isTreino && turmaIds.length"
              :to="`/frequencia?turma=${turmaIds[0]}`"
              class="px-4 py-2 rounded-lg text-sm font-semibold text-white bg-emerald-500 hover:bg-emerald-600 transition-colors"
              @click="$emit('close')"
            >
...
const { iconeEmoji } = useTurmaVisual()
