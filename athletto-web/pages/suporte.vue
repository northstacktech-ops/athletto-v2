<template>
  <div :class="logado ? '' : 'min-h-screen bg-gray-50 dark:bg-surface-canvas-dark py-10 px-4'">
    <div :class="['space-y-4 animate-fade-in', logado ? '' : 'max-w-5xl mx-auto']">
    ...
    <div class="flex items-start justify-between gap-3">
      ...
      <NuxtLink v-if="!logado" to="/" class="shrink-0 text-xs font-semibold text-gray-500 hover:text-gray-700 inline-flex items-center gap-1 pt-1">
...
// Página dupla: pública (rodapé do login, sem layout) e interna (clicada no
// menu por um gestor logado → usa o shell do painel). O layout é decidido em
// middleware para ser SSR-safe.
definePageMeta({
  layout: false,
  middleware: [
    () => {
      if (useSupabaseUser().value) setPageLayout('default')
    },
  ],
})
useHead({ title: 'Suporte — Athletto' })

// Logado → renderiza dentro do shell (sem fundo/padding próprios nem toast duplicado).
const logado = computed(() => !!useSupabaseUser().value)
...
async function enviarChamado() {
  ...
  try {
    const { error } = await supabase.from('chamados_suporte').insert({
      clube_id: clube.value?.id ?? null,
      gestor_id: gestor.value?.id ?? null,
      nome: gestor.value?.nome ?? null,
      email: gestor.value?.email ?? null,
    ...
    chamado.descricao = ''
    chamado.assunto = ''
    toast.success('Chamado enviado!', 'Nossa equipe responde em até 1h útil.')
  } catch (err: any) {
    toast.error('Não foi possível enviar', err?.message ?? 'Tente pelo WhatsApp ao lado.')
  } finally {
...
const faq: { cat: Categoria; q: string; a: string }[] = [
  ...
  },
  {
    ...
    a: 'Quando um planejamento é ativado, o sistema gera um código Pix via ValidaPay para cada atleta. O responsável recebe o código pelo app ou WhatsApp, paga via Pix e o status da cobrança atualiza automaticamente para "Pago" — sem ação manual sua.',
  },
  ...
  // Conta & Plano
  {
    ...
    a: 'Sim. O plano Base permite 1 gestor; o Pro, até 3; e o Elite, gestores ilimitados. Convide em Configurações → aba "Gestores" → "Convidar gestor" — o convidado define a senha em /primeiro-acesso.',
  },
  {
    cat: 'conta',
