  ...
  <div>
    ...
    <form novalidate @submit.prevent="handleSubmit">
      ...
      <div v-if="step === 1" class="space-y-3 animate-fade-up">
        ...
        <!-- Plano selector -->
        <div>
          <label class="auth-label">Plano inicial</label>
          <div class="grid grid-cols-3 gap-2">
            <label
              v-for="plano in planos"
              :key="plano.value"
              class="flex flex-col items-center text-center px-3 py-3 rounded-xl border cursor-pointer transition-all"
              :class="form.plano === plano.value
            ...
            >
              <input
                ...
                class="sr-only"
              />
              <span class="text-sm font-bold text-gray-900">{{ plano.label }}</span>
              <span
                v-if="plano.tag"
                class="mt-1 inline-flex items-center px-1.5 py-0.5 rounded text-xs font-bold"
                :class="plano.tag === 'Grátis 14 dias' ? 'bg-[#ccff00]/30 text-gray-700' : 'bg-brand-100 text-brand-700'"
              >
                {{ plano.tag }}
              </span>
              <span v-else class="mt-1 h-[18px]" />
              <span class="mt-2 text-base font-extrabold text-gray-900">{{ plano.preco.split('/')[0] }}</span>
              <span class="text-xs text-gray-400">/mês</span>
              <span class="mt-1.5 text-xs text-gray-400 leading-tight">{{ plano.desc }}</span>
            </label>
          </div>
        </div>
      </div>
...
// ── Planos ────────────────────────────────────────────────────────────────────
const planos = [
  { value: 'basico', label: 'Básico', preco: 'R$49/mês', tag: 'Grátis 14 dias', desc: 'Até 40 atletas · 5 turmas' },
  { value: 'intermediario', label: 'Intermediário', preco: 'R$99/mês', tag: 'Popular', desc: 'Até 80 atletas · 20 turmas' },
  { value: 'profissional', label: 'Profissional', preco: 'R$199/mês', tag: '', desc: 'Atletas ilimitados · 20 turmas' },
]
