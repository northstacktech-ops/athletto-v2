...
async function fetchDashboard() {
  loading.value = true
  try {
    const [metricsRes, graficoRes, turmasRes, alertasRes] = await Promise.all([
      dashboard.buscarMetricas(),
      dashboard.buscarGrafico(periodMonths.value),
      dashboard.turmasHoje(),
      dashboard.alertasEvasao(4),
    ])
    if (metricsRes.data)  metrics.value = metricsRes.data
    if (graficoRes.data)  graficoFinanceiro.value = graficoRes.data
    if (turmasRes.data)   turmasHoje.value = turmasRes.data as Turma[]
    if (alertasRes.data)  alertasEvasao.value = alertasRes.data as AlertaEvasao[]
  } finally {
    loading.value = false
  }
}
