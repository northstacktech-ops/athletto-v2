...
export default {
  ...
  plugins: [
    // Preline v4 mira o Tailwind v4 (config via CSS) e não traz plugin p/ o
    // Tailwind v3 usado aqui. O JS do Preline adiciona `.open` no overlay
    // aberto; registramos a variante usada pelos componentes (ex.: o drawer
    // mobile `hs-overlay-open:translate-x-0`) para o CSS de abertura existir.
    plugin(({ addVariant }) => {
      addVariant('hs-overlay-open', ['&.open', '.hs-overlay.open &'])
    }),
  ],
} satisfies Config
