 ...
 * geradas, em batches, respeitando rate limit do AbacatePay.
 *
 * Fluxo:
 ...
 *     com 250ms entre batches (margem de rate limit AbacatePay).
...
export default defineEventHandler(async (event) => {
  ...
  for (let i = 0; i < cobrancas.length; i += batchSize) {
    ...
    const results = await Promise.allSettled(
      batch.map((cb) => criarPixParaCobranca(supabase, cb.id, { apiUrl, apiKey })),
    )
