<template>
  <div class="space-y-4 animate-fade-in">
    ...
    <!-- Filtros -->
    <div data-tour="turmas-filtros" class="bg-white dark:bg-surface-elevated-dark rounded-xl border border-slate-200 dark:border-white/[0.10] p-3">
    ...
    <!-- GRID -->
    <div v-else-if="vista === 'grid'" data-tour="turmas-lista" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      <TurmasCard
...
const tour = useGuidedTour('athletto_tour_turmas_done', [
  { tourId: 'turmas-nova', title: 'Nova turma', description: 'Ao criar uma turma, a mensalidade já é configurada junto — os atletas que você vincular são cobrados automaticamente, sem passo extra.' },
  { tourId: 'turmas-filtros', title: 'Buscar e filtrar', description: 'Busque por nome, local ou descrição, e filtre entre turmas ativas e inativas.' },
  { tourId: 'turmas-lista', title: 'Ver detalhes', description: 'Clique numa turma pra ver os atletas vinculados, frequência e o financeiro dela.' },
])
const busca = ref('')
