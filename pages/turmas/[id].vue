  ...
  <div v-else class="space-y-4 animate-fade-in">
    ...
    <div class="bg-white dark:bg-surface-elevated-dark rounded-xl border border-slate-200 dark:border-white/[0.10] shadow-card p-4 flex flex-col sm:flex-row sm:items-center gap-4">
      ...
      <div class="flex items-center gap-2">
        <button v-if="podeEditar" class="px-3 py-2 rounded-lg text-sm font-semibold border border-slate-200 dark:border-white/[0.10] text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/[0.05]" @click="abrirEdicao = true">
          Editar turma
        </button>
        <button v-if="podeEditar" class="px-3 py-2 rounded-lg text-sm font-semibold bg-brand-600 hover:bg-brand-700 text-white transition-colors" @click="abrirVincular = true">
          Vincular atletas
        </button>
        <NuxtLink v-if="temPermissao('frequencia', 'editar')" :to="`/frequencia?turma=${turma.id}`" class="px-3 py-2 rounded-lg text-sm font-semibold bg-emerald-500 hover:bg-emerald-600 text-white transition-colors">
      ...
      <div class="p-3">
        ...
        <div v-else-if="aba === 'atletas'">
          <div class="flex items-center justify-between mb-3">
            ...
            <button v-if="podeEditar" class="px-3 py-1.5 rounded-lg text-xs font-semibold bg-brand-600 hover:bg-brand-700 text-white transition-colors inline-flex items-center gap-1.5" @click="abrirVincular = true">
          ...
          <div v-if="atletasDaTurma.length === 0" class="text-center py-12">
            ...
            <button v-if="podeEditar" class="mt-3 px-4 py-2 rounded-lg text-sm font-semibold bg-brand-600 hover:bg-brand-700 text-white transition-colors" @click="abrirVincular = true">
        ...
        <div v-else-if="aba === 'frequencia'" class="space-y-6">
          ...
          <div>
            ...
            <ul v-else class="divide-y divide-slate-100 dark:divide-white/[0.06]">
              <li v-for="d in datasRegistradas.slice(0, 12)" :key="d.data">
                <button class="w-full py-2.5 flex items-center gap-3 text-sm text-left hover:bg-slate-50 dark:hover:bg-white/[0.02] -mx-2 px-2 rounded-lg" @click="treinoAberto = treinoAberto === d.data ? null : d.data">
                  <svg class="w-4 h-4 text-slate-400 shrink-0 transition-transform" :class="{ 'rotate-90': treinoAberto === d.data }" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>
                  <span class="font-semibold text-slate-900 dark:text-white flex-1">{{ formatDate(d.data) }}</span>
                  <span class="text-slate-500">{{ d.presentes }}/{{ d.total }} presentes</span>
                  <span class="font-bold w-12 text-right" :class="d.percent >= 80 ? 'text-emerald-600' : d.percent >= 60 ? 'text-amber-600' : 'text-red-600'">
                    {{ d.percent }}%
                  </span>
                </button>
                <!-- Drill-down: quem foi / quem faltou -->
                <div v-if="treinoAberto === d.data" class="pb-3 pl-7 grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                  <div v-for="r in presencasDoTreino(d.data)" :key="r.atleta.id" class="flex items-center gap-2 text-sm py-1">
                    <UiAvatar :nome="r.atleta.nome" :src="r.atleta.foto_url" size="xs"/>
                    <span class="flex-1 min-w-0 truncate text-slate-700 dark:text-slate-200">{{ r.atleta.nome }}</span>
                    <span class="text-xs font-bold shrink-0" :class="r.presente ? 'text-emerald-600' : 'text-red-600'">{{ r.presente ? 'Presente' : 'Falta' }}</span>
                  </div>
                </div>
              </li>
            </ul>
                ...
                <div v-if="editandoAtletaId !== a.id" class="py-3 flex items-center gap-3">
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-1.5 mt-0.5">
                      ...
                      <span
                        v-if="a.valor_mensalidade !== 0"
                        class="text-xs font-bold px-1.5 py-0.5 rounded"
                        :class="{
                          'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300': statusPag(a.id) === 'em_dia',
                          'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300': statusPag(a.id) === 'pendente',
                          'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-300': statusPag(a.id) === 'atrasado',
                        }"
                      >
                        {{ statusPag(a.id) === 'em_dia' ? 'Em dia' : statusPag(a.id) === 'pendente' ? 'Pendente' : 'Atrasado' }}
                      </span>
                    </div>
                  </div>
...
// Gestor adicional sem nível 'editar' em turmas só visualiza o detalhe.
const podeEditar = computed(() => temPermissao('turmas', 'editar'))

// Status de pagamento por atleta (em_dia | pendente | atrasado).
const statusPagamento = ref<Record<string, 'em_dia' | 'pendente' | 'atrasado'>>({})
function statusPag(id: string) {
  return statusPagamento.value[id] ?? 'em_dia'
}
...
const caixinhaId = ref<string | null>(null)
const abrirEdicao = ref(false)
const abrirVincular = ref(false)

type TabValue = 'visao' | 'atletas' | 'frequencia' | 'financeiro' | 'configurar'
// "Configurar" só edita dados da turma → escondida de quem não tem 'editar'.
const tabs = computed(() => [
  ...
  ...(podeEditar.value ? [{ value: 'configurar' as const, label: 'Configurar' }] : []),
])
...
const abaInicial = hashToTab[route.hash] ?? 'visao'
// Bloqueia abrir #configurar via URL sem permissão de escrita.
const aba = ref<TabValue>(abaInicial === 'configurar' && !podeEditar.value ? 'visao' : abaInicial)
...
// Drill-down de um treino: presença de cada atleta naquela data.
const treinoAberto = ref<string | null>(null)
function presencasDoTreino(data: string) {
  return freqs.value
    .filter((f) => f.data === data)
    .map((f) => ({ atleta: atletasDaTurma.value.find((a) => a.id === f.atleta_id), presente: f.presente }))
    .filter((x): x is { atleta: Atleta; presente: boolean } => !!x.atleta)
    .sort((a, b) => Number(a.presente) - Number(b.presente) || a.atleta.nome.localeCompare(b.atleta.nome))
}
