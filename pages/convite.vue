          ...
          <ul v-else class="divide-y divide-slate-100 dark:divide-white/[0.06]">
            <li v-for="ind in historico" :key="ind.id" class="px-5 py-3 flex items-center gap-3">
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-slate-400 truncate">{{ formatRelativeDate(ind.criado_em) }} · {{ ind.email_indicado }}</p>
              </div>
              <span class="px-2 py-0.5 rounded-md text-xs font-semibold capitalize shrink-0 whitespace-nowrap" :class="statusCor(ind.status)">{{ ind.status }}</span>
            </li>
          </ul>
        </div>
      </div>
