<template>
  <div class="space-y-4 animate-fade-in">
    ...
    <div class="card-base p-3 flex items-center gap-2 flex-wrap">
      <select v-model="filtroAcao" class="px-3 py-2 rounded-lg border border-slate-200 dark:border-white/[0.10] bg-white dark:bg-surface-canvas-dark text-sm font-medium">
      ...
      <input v-model="filtroDesde" type="date" class="px-3 py-2 rounded-lg border border-slate-200 dark:border-white/[0.10] bg-white dark:bg-surface-canvas-dark text-sm"/>
      <input v-model="filtroAte" type="date" class="px-3 py-2 rounded-lg border border-slate-200 dark:border-white/[0.10] bg-white dark:bg-surface-canvas-dark text-sm"/>
      <button class="text-xs font-semibold text-slate-500 hover:text-slate-700" @click="limpar">Limpar</button>
    </div>
    ...
    <div class="card-base overflow-hidden">
      ...
      <ul v-else class="divide-y divide-slate-100 dark:divide-white/[0.07]">
        <li v-for="log in logs" :key="log.id" class="px-5 py-3 flex items-center gap-3 hover:bg-slate-50 dark:hover:bg-white/[0.02]">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-slate-900 dark:text-white">
              ...
              <span class="font-normal text-slate-500"> — {{ acaoLabel(log.acao) }}</span>
              <code v-if="log.entidade_id" class="text-xs text-slate-400 font-mono ml-1">{{ log.entidade_id }}</code>
            </p>
            <p v-if="hasDetalhes(log.detalhes)" class="text-xs text-slate-500 truncate font-mono">
              {{ JSON.stringify(log.detalhes) }}
            </p>
          </div>
          <span class="text-xs text-slate-400 shrink-0">{{ formatDateTime(log.criado_em) }}</span>
        </li>
      </ul>
    </div>
  </div>
