    ...
    <template v-else-if="cfg">
      ...
      >
        <AdminSettingsField label="Ambiente">
          <select v-model="cfg.validapay_ambiente" class="setting-input">
            <option value="sandbox">Sandbox</option>
            <option value="production">Produção</option>
          </select>
        </AdminSettingsField>
        <AdminSettingsField label="Webhook configurado?" descricao="Lido da env var VALIDAPAY_WEBHOOK_SECRET. Defina em produção para receber baixas automáticas.">
          <span class="px-2 py-1 rounded-md text-xs font-bold uppercase tracking-wider"
                :class="cfg.validapay_webhook_secret_configurado ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'">
            {{ cfg.validapay_webhook_secret_configurado ? 'Configurado' : 'Pendente' }}
          </span>
        </AdminSettingsField>
      </AdminSettingsSection>
