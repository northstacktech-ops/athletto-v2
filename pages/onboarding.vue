...
// Onboarding é uma tela sempre-clara (fundo/branding fixos) e reaproveita
// componentes (ex. BrandAvatarUploader) que têm variantes `dark:` pensadas
// para o shell do painel. Se o SO do usuário está em dark mode, o tema global
// aplica a classe `dark` no <html> e essas variantes ficam quase invisíveis
// aqui (texto/ícone branco translúcido sobre fundo branco). Força modo claro
// enquanto esta página está montada e restaura o tema real ao sair.
const theme = useTheme()
onMounted(() => {
  nextTick(() => document.documentElement.classList.remove('dark'))
})
onBeforeUnmount(() => {
  theme.init()
})
