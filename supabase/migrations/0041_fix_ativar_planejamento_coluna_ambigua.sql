-- ============================================================================
-- Athletto — 0041_fix_ativar_planejamento_coluna_ambigua.sql
--
-- 0040 tinha `update caixinhas set total_pendente = total_previsto` sem
-- qualificar a coluna — Postgres não sabia se `total_previsto` era a
-- variável PL/pgSQL ou a coluna da tabela (ambos existem com esse nome) e
-- rejeitava com "column reference is ambiguous". Qualifica com alias.
-- ============================================================================
...
begin
  ...
  if p.tipo = 'parcelado' then
    ...
    update public.caixinhas cx
     ...
     where cx.id = cx_id;
    update public.caixinhas cx
       set total_pendente = cx.total_previsto
     where cx.id = cx_id;
