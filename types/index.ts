...
export interface Gestor {
  ...
  /** Por módulo: 'ver' | 'editar' (ausente = sem acesso). Principal ignora (acesso total). */
...
export interface Atleta {
  ...
  app_ultimo_acesso?: string | null
  app_senha_definida_em?: string | null
...
export interface Turma {
  ...
  local: string | null
  ativo: boolean
  /** Ícone esportivo da turma (chave do set TurmaIcone). */
  icone?: string
  /** Cor de identidade da turma (brand|emerald|amber|rose|violet). */
  cor?: string
  /** Mensalidade padrão cobrada pra atletas dessa turma (centavos não — número decimal em R$). */
  valor_mensalidade_padrao?: number
  /** Dia do mês (1-28) em que a mensalidade automática desta turma vence. */
...
export type PlanejamentoTipo = 'recorrente' | 'unico' | 'parcelado'
...
export interface Planejamento {
  ...
  criado_em: string
  atualizado_em: string
  /** Não nulo = este é o planejamento de mensalidade automática de uma turma. */
  turma_id?: string | null
  /** Só preenchido quando tipo = 'parcelado'. */
...
// Limites por plano. `null` = ilimitado. `gestores` é o TOTAL (inclui o
// principal) e casa com `max_gestores` do banco (limites_plano) — fonte real
// de enforcement via triggers. Estes valores DEVEM bater com limites_plano.
export interface PlanoLimites {
  atletas: number | null
  turmas: number | null
  gestores: number | null
}

// ── Catálogo único de planos (fonte de verdade de nome, preço e limites) ──────
// `id` é o valor interno do enum plano_clube (não muda); `nome` é o rótulo
// exibido ao usuário. Trocar o nome aqui reflete em todo o app.
export interface PlanoInfo {
  id: Clube['plano']
  nome: string
  preco: number          // mensal, BRL
  tag?: string           // selo (ex.: "Popular")
  destaque?: boolean     // realce visual na comparação
  limites: PlanoLimites
}

export const PLANOS: Record<Clube['plano'], PlanoInfo> = {
  basico: {
    id: 'basico', nome: 'Base', preco: 79,
    limites: { atletas: 30, turmas: 3, gestores: 1 },
  },
  intermediario: {
    id: 'intermediario', nome: 'Pro', preco: 199, tag: 'Popular', destaque: true,
    limites: { atletas: 100, turmas: 10, gestores: 3 },
  },
  profissional: {
    id: 'profissional', nome: 'Elite', preco: 449,
    limites: { atletas: null, turmas: null, gestores: null },
  },
}

export const PLANOS_LISTA: PlanoInfo[] = [
  PLANOS.basico, PLANOS.intermediario, PLANOS.profissional,
]

export function nomePlano(id?: Clube['plano'] | null): string {
  return (id && PLANOS[id]?.nome) || PLANOS.basico.nome
}
export function precoPlano(id?: Clube['plano'] | null): number {
  return (id && PLANOS[id]?.preco) ?? PLANOS.basico.preco
}

// Mapa de limites por id — usado por usePlanoLimites (deriva do catálogo).
export const PLANO_LIMITES: Record<Clube['plano'], PlanoLimites> = {
  basico: PLANOS.basico.limites,
  intermediario: PLANOS.intermediario.limites,
  profissional: PLANOS.profissional.limites,
}
...
// ─── Webhook log (idempotência ValidaPay etc.) ────────────────────────────────

export type WebhookOrigem = 'validapay' | 'outro'
...
export interface ConfiguracaoSistema {
  ...
  // ValidaPay
  validapay_ambiente: 'sandbox' | 'production'
  validapay_webhook_secret_configurado: boolean
  // Limites globais
