...
const ambiente = computed(() =>
  config.public.validapayConfigurado ? 'Produção' : 'Sandbox',
)
const ambienteBg = computed(() =>
  config.public.validapayConfigurado
    ? 'bg-red-50 dark:bg-red-900/20'
    : 'bg-amber-50 dark:bg-amber-900/20',
)
const ambienteDot = computed(() =>
  config.public.validapayConfigurado ? 'bg-red-500' : 'bg-amber-400',
)
...
const TITULOS: Record<string, { titulo: string; subtitulo?: string }> = {
  ...
  '/admin/webhooks':       { titulo: 'Webhooks',      subtitulo: 'Eventos da ValidaPay' },
