...
const emit = defineEmits<{
  (e: 'close'): void
  (e: 'salvo'): void
  /** Upload da foto é imediato (não espera o "Salvar") — avisa o pai na hora
   *  pra ele atualizar o estado local mesmo se o usuário fechar o modal sem
   *  salvar o resto do formulário (senão a foto só aparece após um refresh). */
  (e: 'foto-atualizada', url: string): void
}>()
...
async function onFotoConfirm(file: File) {
  ...
  if (editando.value && props.atleta) {
    ...
    try {
      ...
      emit('foto-atualizada', fotoUrl.value)
