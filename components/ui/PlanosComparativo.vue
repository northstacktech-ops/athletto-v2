...
const props = defineProps<{
  ...
  jaAssinante?: boolean
  /**
   * Clube está em trial (sem pagamento real ainda). O plano do trial aparece
   * em `planoAtual`, mas isso não é uma assinatura paga — o CTA de compra
   * continua ativo mesmo para o plano que já está em uso via trial.
   */
  trialAtivo?: boolean
}>()
...
// Durante o trial, "atual" não conta como assinatura paga — sempre mostra CTA de compra.
const ORDEM = PLANOS_LISTA.map((p) => p.id)
function rotuloBotao(p: typeof PLANOS_LISTA[number]) {
  if (props.planoAtual === p.id && !props.trialAtivo) return 'Plano atual'
