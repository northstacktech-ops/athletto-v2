  ...
  <div class="relative" ref="rootRef">
    ...
    >
      ...
      >
        ...
        <!-- List (preview — só as mais recentes; lista completa em /notificacoes) -->
        <div class="max-h-[400px] overflow-y-auto scrollbar-slim">
          ...
          <ul v-else class="divide-y divide-gray-100 dark:divide-white/[0.04]">
            <li v-for="n in preview" :key="n.id">
              <component
        ...
        <!-- Footer: acesso à tela completa -->
        <NuxtLink
          to="/notificacoes"
          class="block px-4 py-2.5 text-center text-sm font-semibold text-brand-600 dark:text-brand-400 hover:bg-gray-50 dark:hover:bg-white/[0.03] border-t border-gray-100 dark:border-white/[0.06]"
          @click="open = false"
        >
...
// Preview no topbar mostra só as 5 mais recentes — a lista completa fica em /notificacoes.
const PREVIEW_MAX = 5
const preview = computed(() => items.value.slice(0, PREVIEW_MAX))

const iconBg = notifIconBg
const iconColor = notifIconColor
