...
// ── Search + sections ─────────────────────────────────────────
// Ações são atalhos de escrita → exigem nível 'editar' no módulo.
...
].filter(item => !item.modulo || temPermissao(item.modulo, 'editar')))

// Navegação exige nível 'ver'; itens sem `modulo` são sempre visíveis.
const allNav = computed<CmdItem[]>(() => [
  { id: 'n1', label: 'Painel', icon: 'arrow', to: '/' },
  { id: 'n2', label: 'Atletas', icon: 'arrow', to: '/atletas', modulo: 'atletas' },
  { id: 'n3', label: 'Turmas', icon: 'arrow', to: '/turmas', modulo: 'turmas' },
  { id: 'n4', label: 'Frequência', icon: 'arrow', to: '/frequencia', modulo: 'frequencia' },
  { id: 'n5', label: 'Calendário', icon: 'arrow', to: '/calendario', modulo: 'calendario' },
  { id: 'n6', label: 'Financeiro', icon: 'arrow', to: '/financeiro', modulo: 'financeiro' },
  { id: 'n7', label: 'Configurações', icon: 'arrow', to: '/configuracoes', modulo: 'configuracoes' },
...
const results = computed<CmdSection[]>(() => {
  ...
  // Atletas (only when querying e com permissão de ver o módulo)
  if (q.length > 0 && temPermissao('atletas')) {
  ...
  // Turmas (only when querying e com permissão de ver o módulo)
  if (q.length > 0 && temPermissao('turmas')) {
