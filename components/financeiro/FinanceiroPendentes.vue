<template>
  <div class="space-y-4">
    ...
    <div class="bg-white dark:bg-surface-elevated-dark rounded-xl border border-slate-200 dark:border-white/[0.10] overflow-hidden shadow-card">
      ...
      <ul v-if="filtradas.length > 0" class="divide-y divide-slate-100 dark:divide-white/[0.07]">
        <li v-for="cb in filtradas" :key="cb.id" class="px-5 py-3 flex items-center gap-3 flex-wrap">
          ...
          <div v-if="cb.status === 'pendente'" class="flex items-center gap-1 shrink-0">
            <button v-if="cb.validapay_emv" class="px-2.5 py-1.5 rounded-lg text-xs font-semibold bg-emerald-50 text-emerald-700 hover:bg-emerald-100" @click="copiarPix(cb)">Copiar Pix</button>
      ...
      <div v-if="temMais" class="px-5 py-3 border-t border-slate-100 dark:border-white/[0.06] text-center">
        <button
          type="button"
          class="text-sm font-semibold text-brand-600 hover:text-brand-700 disabled:opacity-50"
...
// Antes buscava TODAS as cobranças do clube (histórico completo, sem teto),
// com join de atleta/caixinha em cada uma — em clubes com bastante tempo de
// uso isso significava milhares de linhas e ~10s pra abrir a aba. Agora pagina
// (ordenado por vencimento — as mais urgentes primeiro) e carrega o resto sob demanda.
async function carregar() {
  loading.value = true
  const { data } = await fin.listarCobranças({ limite: PAGINA })
  todas.value = (data ?? []) as Cobranca[]
  temMais.value = (data?.length ?? 0) === PAGINA
  loading.value = false
}
...
async function carregarMais() {
  ...
  const { data } = await fin.listarCobranças({ limite: PAGINA, offset: todas.value.length })
  todas.value = [...todas.value, ...(data ?? []) as Cobranca[]]
  temMais.value = (data?.length ?? 0) === PAGINA
...
function linkWhatsApp(cb: Cobranca) {
  ...
  const msg = `Oi! Lembrete da mensalidade de ${cb.atleta?.nome} (${formatCurrency(cb.valor)}), vencimento ${formatDate(cb.data_vencimento)}. ${cb.validapay_emv ? `Pix copia-e-cola: ${cb.validapay_emv}` : ''}`
  return gerarLinkWhatsApp(fone, msg)
}

async function copiarPix(cb: Cobranca) {
  if (!cb.validapay_emv) return
  try {
    await navigator.clipboard.writeText(cb.validapay_emv)
    toast.success('Código Pix copiado', 'Cole no app do banco do responsável.')
  } catch {
    toast.error('Não foi possível copiar', 'Copie manualmente o código Pix.')
  }
}
...
async function regenerar(id: string) {
  try {
    ...
    toast.success('Pix regenerado', 'Copie o novo código e envie ao responsável.')
    await carregar()
  } catch (err: any) {
    toast.error('Falha ao regenerar Pix', err?.message ?? '')
  }
}
