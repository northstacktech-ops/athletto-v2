        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="flex items-center gap-1.5">
              ...
              <span v-if="ehMensalidadeDeTurma(cx)" class="shrink-0 px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide bg-brand-50 text-brand-700 dark:bg-brand-500/10 dark:text-brand-300">
                Mensalidade
              </span>
            </div>
            <p class="text-xs text-slate-500 truncate mt-0.5">
              {{ ehMensalidadeDeTurma(cx) ? 'Gerenciada pela turma — desative a turma pra encerrar' : `Criada ${formatDate(cx.criada_em.slice(0, 10))}` }}
            </p>
          </div>
...
// Caixinha nascida de mensalidade de turma (planejamentos.turma_id não nulo) —
// não é criada/encerrada manualmente aqui, só reflete o que a turma configura.
function ehMensalidadeDeTurma(cx: Caixinha) {
  return !!(cx as any).planejamentos?.turma_id
}
