        ...
        <div v-if="passo === 1" class="px-6 py-5 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <div class="grid grid-cols-3 gap-2">
              ...
              <button type="button" class="py-2.5 rounded-lg text-sm font-semibold border transition-colors"
                :class="form.tipo === 'parcelado' ? 'bg-slate-900 text-white border-slate-900' : 'bg-white dark:bg-surface-canvas-dark text-gray-600 border-gray-200 dark:border-white/[0.10] hover:bg-gray-50'"
                @click="form.tipo = 'parcelado'">
                Parcelado
              </button>
            </div>
            <p v-if="form.tipo === 'parcelado'" class="text-xs text-gray-500 mt-1.5">
              Valor total dividido em N parcelas mensais. Ex: viagem, campeonato — cada atleta paga uma parcela por mês.
            </p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                {{ form.tipo === 'parcelado' ? 'Valor total *' : 'Valor padrão por atleta *' }}
              </label>
              <div class="relative">
            ...
            <div v-else>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Número de parcelas</label>
              <input v-model.number="form.numero_parcelas" type="number" min="1" step="1" class="form-input"/>
            </div>
          </div>
          ...
          <div v-if="form.tipo === 'parcelado'" class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Data de início</label>
              <input v-model="form.data_inicio" type="date" class="form-input"/>
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Dia de vencimento (1 – 28)</label>
              <input v-model.number="form.dia_vencimento" type="number" min="1" max="28" class="form-input"/>
          ...
          <p v-if="form.tipo === 'parcelado' && form.valor > 0 && form.numero_parcelas > 0" class="text-xs text-gray-500">
            {{ form.numero_parcelas }}x de {{ formatCurrency(form.valor / form.numero_parcelas) }} por atleta
          </p>
        </div>
...
const podeProsseguir = computed(() => {
  if (passo.value === 1) {
    if (form.nome.trim().length < 3 || form.valor <= 0) return false
    if (form.tipo === 'parcelado') return form.numero_parcelas > 0 && !!form.data_inicio && form.dia_vencimento >= 1 && form.dia_vencimento <= 28
    return true
  }
...
async function salvarComo(estadoInicial: 'inativo' | 'ativo') {
  ...
  try {
    const { data: planej, error } = await fin.criarPlanejamento({
      ...
      dia_vencimento: form.tipo === 'recorrente' || form.tipo === 'parcelado' ? form.dia_vencimento : null,
      data_vencimento: form.tipo === 'unico' ? form.data_vencimento : null,
      numero_parcelas: form.tipo === 'parcelado' ? form.numero_parcelas : null,
      data_inicio: form.tipo === 'parcelado' ? form.data_inicio : null,
      status: 'inativo',
      ativado_em: null,
      encerrado_em: null,
    })
