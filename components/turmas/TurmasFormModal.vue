      ...
      <div class="relative w-full sm:max-w-xl bg-white dark:bg-surface-elevated-dark rounded-t-2xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <form class="px-6 py-5 space-y-4 overflow-y-auto" @submit.prevent="salvar">
          ...
          <div>
            ...
            <select v-model.number="form.dia_vencimento_mensalidade" required class="form-input max-w-[120px]">
              <option v-for="d in 28" :key="d" :value="d">{{ d }}</option>
            </select>
            <p class="text-xs text-gray-500 mt-1">
              A mensalidade é cobrada automaticamente todo mês, neste dia, pros atletas desta turma.
            </p>
          </div>
        </form>
...
const form = reactive({
  ...
  dia_vencimento_mensalidade: props.turma?.dia_vencimento_mensalidade ?? 10,
})
...
async function salvar() {
  ...
  try {
    ...
    if (editando.value && props.turma) {
      ...
      const { mensalidadeError } = await turmasComp.criar(payload)
      toast.success('Turma criada')
      if (mensalidadeError) {
        toast.error(
          'Mensalidade não pôde ser configurada',
          'A turma foi criada, mas a cobrança automática falhou ao ser configurada. Fale com o suporte.',
        )
      }
    }
    emit('salvo')
