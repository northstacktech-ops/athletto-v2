...
export function useAtletas() {
  ...
  // Passa pela server route (não upsert direto): a turma pode ter mensalidade
  // ativa, e nesse caso o vínculo precisa gerar a cobrança do ciclo atual +
  // criar o Pix na hora (RPC atleta_entrar_turma + criarPixParaCobranca no
  // server, mesmo padrão de /api/planejamentos/[id]/ativar).
  async function vincularTurma(atleta_id: string, turma_id: string) {
    try {
      const session = await supabase.auth.getSession()
      const token = session.data.session?.access_token
      await $fetch(`/api/turmas/${turma_id}/entrar-atleta`, {
        method: 'POST',
  ...
  // RPC atleta_sair_turma cuida de desvincular + cancelar cobranças futuras
  // pendentes da mensalidade da turma (histórico passado fica intacto).
  async function desvincularTurma(atleta_id: string, turma_id: string) {
    const { error } = await supabase.rpc('atleta_sair_turma', {
