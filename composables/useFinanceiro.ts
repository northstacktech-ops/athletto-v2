...
export function useFinanceiro() {
  ...
  // Prévia do espelho: quantos atletas/cobranças futuras seriam atualizados ao
  // mudar o valor do planejamento (só quem segue o padrão).
  async function previewPropagacao(planejamento_id: string, novo_valor: number) {
    const { data, error } = await supabase.rpc('preview_propagacao_planejamento', {
      p_planejamento_id: planejamento_id,
      p_novo_valor: novo_valor,
    })
    return { data: data as { atletas: number; cobrancas: number } | null, error }
  }

  // Aplica a propagação do valor (espelho). Retorna nº de cobranças atualizadas.
  async function propagarValor(planejamento_id: string, novo_valor: number) {
    const { data, error } = await supabase.rpc('propagar_valor_planejamento', {
      p_planejamento_id: planejamento_id,
      p_novo_valor: novo_valor,
    })
  ...
  // Adiciona um atleta a uma caixinha parcelada já em andamento. `modo`:
  // 'parcial' (só as parcelas que restam, valor de parcela igual ao de quem
  // já está desde o início) ou 'integral' (valor total do plano, comprimido
  // nas parcelas que restam).
  async function adicionarAtletaCaixinhaParcelada(
    planejamento_id: string,
    atleta_id: string,
    modo: 'parcial' | 'integral',
  ) {
    const { data, error } = await supabase.rpc('adicionar_atleta_caixinha_parcelada', {
      p_planejamento_id: planejamento_id,
      p_atleta_id: atleta_id,
      p_modo: modo,
    })
    return { data: data as { parcelas_geradas: number; valor_parcela: number; modo: string } | null, error }
  }

  async function ativarPlanejamento(id: string) {
    // A ativação é feita pela server route que:
    //  1) chama o RPC ativar_planejamento
    //  2) gera Pix na ValidaPay em batch
  ...
  async function listarAtletasPlanejamento(planejamento_id: string) {
    const { data, error } = await supabase
      ...
      .select('atleta_id, valor_customizado, isento, atletas(id, nome, apelido, foto_url, valor_mensalidade, status)')
  ...
  async function listarCaixinhas() {
    const { data, error } = await supabase
      .from('caixinhas')
      .select('*, planejamentos(id, nome, valor, tipo, periodicidade, dia_vencimento, turma_id, numero_parcelas, data_inicio)')
      .eq('clube_id', getClubId())
  ...
  async function regenerarLink(id: string) {
    // Limpa o Pix antigo no banco e dispara nova criação via ValidaPay
  ...
  // Resumo do Dashboard Financeiro — tudo agregado no Postgres em UMA
  // chamada (receita/despesa do mês, a receber, status das cobranças e
  // despesas por categoria). Substitui os fetches de transações/cobranças
  // sem limite que existiam antes e somavam no navegador.
  ...
  async function buscarResumoDashboard(mesesCategoria = 6): Promise<{
    data: FinanceiroResumo | null
    error: Error | null
  }> {
    const { data, error } = await supabase.rpc('financeiro_resumo', {
      p_clube_id: getClubId(),
      p_meses_categoria: mesesCategoria,
    })
    return { data: data as FinanceiroResumo | null, error: error as Error | null }
  }
  ...
  return {
    ...
    previewPropagacao,
    propagarValor,
    adicionarAtletaCaixinhaParcelada,
    ativarPlanejamento,
