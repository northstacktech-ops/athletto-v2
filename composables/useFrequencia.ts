...
export function useFrequencia() {
  ...
  async function atletasDaTurma(turma_id: string) {
    // `atletas!inner` é obrigatório aqui: sem o `!inner`, o PostgREST trata o
    // filtro em `atletas.status`/`atletas.ativo` como um LEFT JOIN — quando o
    // atleta vinculado está afastado ou inativo, a linha de atleta_turma NÃO
    // é descartada, ela volta com `atletas: null`. Isso derrubava as telas
    // que consomem esse retorno (Avatar recebia nome/foto undefined e
    // quebrava o render). Com `!inner`, essas linhas são corretamente
    // excluídas do resultado.
    const { data, error } = await supabase
      .from('atleta_turma')
      .select('atletas!inner(id, nome, apelido, foto_url, numero_camisa, posicao, status, ativo, valor_mensalidade, telefone_responsavel)')
      .eq('turma_id', turma_id)
      .eq('ativo', true)
